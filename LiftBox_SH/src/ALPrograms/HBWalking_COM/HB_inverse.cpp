...
HB_inverse::HB_inverse()
{
    ...
    //SH_EDIT JH_COMMENT: PELVIS COM MOVE
//    if(sharedData->walkingMode == 1)
    c_pel = vec3(0.07, -0.00, 0.18);
        //c_pel = vec3(0.05, -0.00, 0.18); //SH_ORIGINAL BEFORE 10/7/2020
//    else
//        c_pel = vec3(0.00, -0.00, 0.20);  //ORIGINAL HBWalking code
...
vec3 HB_inverse::FKCOM_UB(ArmJoints AJ)
{
    vec3 COM_UB;
...
ArmPos HB_inverse::FK_UB(ArmJoints AJ){
    ArmPos ap;
    ap.qPel = AJ.qPel;
    ap.pPel = AJ.pPel;
...
FeetPos HB_inverse::FK(LegJoints LJ)
{
    FeetPos rp;
    rp.qPel = LJ.qPel;
    rp.pPel = LJ.pPel;
...
LegJoints HB_inverse::IK_pel(vec3 pPel, quat qPel, vec3 pRF, quat qRF, vec3 pLF, quat qLF)
{
    LegJoints LJ;
...
LegJoints HB_inverse::IK_pel_drc(vec3 pPel, quat qPel, vec3 pRF, quat qRF, vec3 pLF, quat qLF)
{
    LegJoints LJ;
...
LegJoints HB_inverse::IK_COM(vec3 pCOM, quat qPel, vec3 pRF, quat qRF, vec3 pLF, quat qLF)
{
...
LegJoints HB_inverse::IK_COM_xy(vec3 pCOM_des, double pPelz, quat qPel, vec3 pRF, quat qRF, vec3 pLF, quat qLF)
{
