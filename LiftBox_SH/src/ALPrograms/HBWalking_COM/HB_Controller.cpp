...
vec4 HB_PreviewWalk::Ankle_Torque_from_cZMP(vec3 _global_cZMP, vec3 _global_ZMP, quat _global_qRF, quat _global_qLF,
                                            vec3 _global_pRF, vec3 _global_pLF, vec3 _F_RF, vec3 _F_LF, vec3 _COM_global){
    ...
    vec3 local_COM = global2local_point(_global_qRF, _global_qLF, _global_pRF, _global_pLF, _COM_global);
    ...
    Point P_local_cZMP = {local_COM.x, local_COM.y}; // new change
...
void HB_PreviewWalk::AnkleTorqueController_pos(double _RAR_T_ref, double _RAP_T_ref, double _LAR_T_ref, double _LAP_T_ref,
    ...
    //// variable gains
    double gain_roll_max = -0.2;//-0.5; //new change
    double gain_roll_min = -0.01;//-0.035;

    double gain_pitch_max = -0.2;//-0.5;//-0.5; //new change
...
//    RF_variable_gain_pitch = gain_pitch_air*1/(1 + _F_RF_filtered_new.z/decrease_factor_pitch);
    ...
    RF_variable_gain_roll = (gain_roll_min - gain_roll_max)/250*_F_RF_filtered_new.z + gain_roll_max;
    RF_variable_gain_pitch = (gain_pitch_min - gain_pitch_max)/250*_F_RF_filtered_new.z + gain_pitch_max;
...
//    //cout<<"roll gain : "<<LF_varible_gain_roll<<"  pitch gain : "<<LF_variable_gain_pitch<<endl;
    ...
    LF_variable_gain_roll = (gain_roll_min - gain_roll_max)/250*_F_LF_filtered_new.z + gain_roll_max;
    LF_variable_gain_pitch = (gain_pitch_min - gain_pitch_max)/250*_F_LF_filtered_new.z + gain_pitch_max;
...
//    Alpha_dsp = (A + 1.0)/2.0;
    ...
    if(step_phase <= 1 || step_phase > N_step - 2){
        ...
        dsp_tilt_gain = 2.5;//0.9 //new change
    }
    else{
        dsp_ctrl_gain = 0.00015;// new change //0.0012
        dsp_tilt_gain = 4.0; //10 //new change
    }
    ...
    if(Standing_mode_flag == true){
        ...
        if(Alpha_dsp < 0.01)Alpha_dsp = 0.01;
        if(Alpha_dsp > 0.99)Alpha_dsp = 0.99; //new change
    }
...
//        Pelv_roll_vel_ref = 0*COM_e_imu_local.y - Pelv_roll_ref/0.5;
        ...
        dz_ctrl = dsp_ctrl_gain/2.0*Fz_diff_error + dsp_tilt_gain/3.0*COM_e_imu_local.y
                    +0*L*dsp_tilt_gain*COM_e_imu_local.x - z_ctrl/0.2;
...
//    if(Alpha_landing <= 0.0) Alpha_landing = 0.0;
    ...
    double RF_Threshold = Alpha_AT*Fz_total - 100; //new change
    double LF_Threshold = (1.0 - Alpha_AT)*Fz_total - 100;
...
void HB_PreviewWalk::WindowFill_1st(){
    int last_tic;
    ...
    if(dT >= SDB[step_phase].t - t_half_dsp){
        ...
        double Ay,By, Ax,Bx;
        if(step_phase >= 1){
            calc_1st_param(0, t_half_dsp*2, SDB[step_phase-1].Fpos.y, SDB[step_phase].Fpos.y, Ay,By);
            calc_1st_param(0, t_half_dsp*2, SDB[step_phase-1].Fpos.x, SDB[step_phase].Fpos.x, Ax,Bx);
        }
        else{
            calc_1st_param(0, t_half_dsp*2, 0, SDB[step_phase].Fpos.y,Ay,By);
            calc_1st_param(0, t_half_dsp*2, 0, SDB[step_phase].Fpos.x,Ax,Bx);
        }
        ...
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase].Fpos.y, SDB[step_phase + 1].Fpos.y,Ay,By);
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase].Fpos.x, SDB[step_phase + 1].Fpos.x,Ax,Bx);
        dsp_timer = 0;
...
//            if((int)((dT + dt/2)*freq) == 2) cout<<"dsp timer2"<<dsp_timer<<endl;
    ...
    for(int j=1; j<= No_of_cycle; j++){
        ...
        double Ay,By, Ax,Bx;
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + j - 1].Fpos.y, SDB[step_phase + j].Fpos.y,Ay,By);
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + j - 1].Fpos.x, SDB[step_phase + j].Fpos.x,Ax,Bx);
        ...
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + j].Fpos.y, SDB[step_phase + j + 1].Fpos.y,Ay,By);
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + j].Fpos.x, SDB[step_phase + j + 1].Fpos.x,Ax,Bx);
    ...
    if((int)((t_prev + dt/2)*freq) + 1 - last_tic  < (int)((t_half_dsp + dt/2)*freq)){
        ...
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + No_of_cycle].Fpos.y, SDB[step_phase + No_of_cycle + 1].Fpos.y, Ay,By);
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + No_of_cycle].Fpos.x, SDB[step_phase + No_of_cycle + 1].Fpos.x, Ax,Bx);
        ...
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + No_of_cycle + 1].Fpos.y, SDB[step_phase + No_of_cycle + 2].Fpos.y, Ay,By);
        calc_1st_param(0, t_half_dsp*2, SDB[step_phase + No_of_cycle + 1].Fpos.x, SDB[step_phase + No_of_cycle + 2].Fpos.x, Ax,Bx);
...
void HB_PreviewWalk::WindowFill_SA_ref_1st(){
    int last_tic;
    double t_prev_plus_t_step = t_prev + t_step;
