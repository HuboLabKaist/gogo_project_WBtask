...
void TaskMotion::ResetGlobalCoord(int RF_OR_LF_OR_PC){
    ...
    //SH_EDIT
    if(Shm->Flag_COMreset)
    {
        Q0_x = 0;
        Q0_y = 0;
        Shm->Flag_COMreset = false;
    }
    else
    {
        Q0_x += Q_filt_34x1[0];
...
void TaskMotion::RefreshToCurrentReference(void){
    ...
    double tempZ = Q_filt_34x1[idZ]; //SH_EDIT
    Q_filt_34x1[idZ] = 0.0;          //SH_EDIT
    ...
    Q_filt_34x1[idZ] = tempZ;       //SH_EDIT

    kine_drc.FK_COM_Global(Q_filt_34x1, pCOM_2x1);

    WSTrotApplied();            //SH_EDIT
    pPelX = Q_filt_34x1[idX];   //SH_EDIT
    pPelY = Q_filt_34x1[idY];   //SH_EDIT
    ...
    //wbPosWST->SetRetValue(Q_filt_34x1[WST]*D2R); //ORIGINAL_CODE
    wbPosWST->SetRetValue(Q_filt_34x1[idWST]*R2D); //SH_EDIT
...
void TaskMotion::WSTrotApplied(void)
{
    //Hand x,y,z coordinate
    double pRHx = pRH_3x1[0];    double pRHy = pRH_3x1[1];
    double pLHx = pLH_3x1[0];    double pLHy = pLH_3x1[1];

    pRH_3x1[0]  = pRHx * cos(-Q_filt_34x1[idWST]) - pRHy * sin(-Q_filt_34x1[idWST]);
    pRH_3x1[1]  = pRHx * sin(-Q_filt_34x1[idWST]) + pRHy * cos(-Q_filt_34x1[idWST]);

    pLH_3x1[0]  = pLHx * cos(-Q_filt_34x1[idWST]) - pLHy * sin(-Q_filt_34x1[idWST]);
    pLH_3x1[1]  = pLHx * sin(-Q_filt_34x1[idWST]) + pLHy * cos(-Q_filt_34x1[idWST]);

    //Hand Quaternion
    QQuaternion q_RH(qRH_4x1[0],qRH_4x1[1],qRH_4x1[2],qRH_4x1[3]);
    QQuaternion q_LH(qLH_4x1[0],qLH_4x1[1],qLH_4x1[2],qLH_4x1[3]);

    QQuaternion q_rot = QQuaternion::fromAxisAndAngle(0.0f, 0.0f, 1.0f, -Q_filt_34x1[idWST]*R2D);
    QQuaternion q_RH_res  = q_rot * q_RH;
    QQuaternion q_LH_res  = q_rot * q_LH;

    qRH_4x1[0] = q_RH_res.scalar();
    qRH_4x1[1] = q_RH_res.x();
    qRH_4x1[2] = q_RH_res.y();
    qRH_4x1[3] = q_RH_res.z();

    qLH_4x1[0] = q_LH_res.scalar();
    qLH_4x1[1] = q_LH_res.x();
    qLH_4x1[2] = q_LH_res.y();
    qLH_4x1[3] = q_LH_res.z();
...
void TaskMotion::WBIK(){
    ...
    kine_drc.IK_WholeBody_Global(des_pCOM_2x1, des_pPELz,   des_qPEL_4x1, des_pRF_3x1, des_qRF_4x1,
                                 des_pLF_3x1,  des_qLF_4x1, des_pRH_3x1,  des_qRH_4x1, des_RElb_ang,
                                 RH_ref_frame, des_pLH_3x1, des_qLH_4x1,  des_LElb_ang,
                                 LH_ref_frame, des_rWST,    Qout_34x1);
