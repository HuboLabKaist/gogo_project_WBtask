...
//                sharedData->Choreonoid.AnkleTorqueCon_flag = true;
        ...
        case HBWalking_PrevWalk:
        {
            ...
            if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[1] == 1){
                cout << "*** WALKING COM BACK MODE ***" << endl;
                sharedData->walkingMode = 1;                    //CHECK initialization!!!!!!!!!!!!! WHERE???
            }
            else
                sharedData->walkingMode = 0;
...
void RBTaskThread(void *)
{
    ...
    //            HBPW.cZMP_TC_proj = zmpProjectionToSP_offset(HBPW.cZMP_TC_global, HBPW.pRF_ref, HBPW.pLF_ref, HBPW.qRF_ref, HBPW.qLF_ref, HBPW.RS.F_RF, HBPW.RS.F_LF, +0.00, +0.00);
                ...
                //HBPW.AnkleTorque_ref = HBPW.Ankle_Torque_from_cZMP(HBPW.cZMP_TC_proj, HBPW.ZMP_global, HBPW.qRF_ref, HBPW.qLF_ref, HBPW.pRF_ref, HBPW.pLF_ref, HBPW.RS.F_RF, HBPW.RS.F_LF);  //new Change
...
//                WBmotion->addCOMInfo_xy_pelz_HB(COM_total.x-0.05, COM_total.y, WBmotion->pPel_3x1[2]);
    ...
    //            cout<<"time : "<<(long)(end - begin)/100000.0<<endl; // 0.1ms unit
                ...
                sharedData->COM_SH_ref[0] = HBPW.COM_ref.x;
                sharedData->COM_SH_ref[1] = HBPW.COM_ref.y;
                sharedData->COM_SH_ref[2] = HBPW.COM_ref.z;
    ...
    */
    ...
    //            cout<<"uCOMy: "<<HBPW.uCOM.y<<"  ZMPy : "<<HBPW.ZMP_global.y<<endl;
                cout<<"uCOMx: "<<HBPW.uCOM.x<<"  ZMPx : "<<HBPW.ZMP_global.x<<endl;
    ...
    */
    ...
    //                fprintf(daemonFile,"%f, ",sharedData->ENCODER[MC_GetID(joint_num)][MC_GetCH(joint_num)].CurrentPosition);
        ...
        if(WB_FLAG == 1)
        {
            ...
            sharedData->zmp_HBWalk[0] = ZMP_global.x;
            sharedData->zmp_HBWalk[1] = ZMP_global.y;
            sharedData->zmp_HBWalk[2] = ZMP_global.z;


            sharedData->zmp_HBWalk_ref[0] = HBPW.p_ref[0].x;
            sharedData->zmp_HBWalk_ref[1] = HBPW.p_ref[0].y;
            sharedData->zmp_HBWalk_ref[2] = ZMP_global.z;
