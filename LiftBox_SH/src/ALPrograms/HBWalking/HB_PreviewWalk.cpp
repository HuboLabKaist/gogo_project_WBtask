...
//    //cout<<"p_ref_offset y huhuhu : "<<tilt_angle_y_filtered<<endl;
    ...
    double scaling_factor = 1.06; //New change
...
//    Using damping con + imu error
    ...
    // Using only imu error
    vec3 temp_cZMP_TC_local = global2local_point(qRF_ref, qLF_ref, pRF_ref, pLF_ref, p_ref[0]) + (1 + 12.0/w)*(COM_e_imu_local + 0.4*dCOM_e_imu_local/w);
...
//        if(CP_error_lf.x < -0.13) CP_error_lf.x = -0.13;
        ...
        if(CP_error_lf.x > 0.20) CP_error_lf.x = 0.20;
        if(CP_error_lf.x < -0.20) CP_error_lf.x = -0.20;
...
//        CP_error_rf.y = CP_error_rf.y*0.9;
        ...
        if(CP_error_rf.x > 0.20) CP_error_rf.x = 0.20;
        if(CP_error_rf.x < -0.20) CP_error_rf.x = -0.20;
...
//    }
    ...
    if(FootUp_height > 0.1) FootUp_height = 0.1;
...
//                cout<<"Lf landing x: "<<pLF_landing.x<<"  pLF_landing.y: "<<pLF_landing.y<<endl;
    ...
    AnkleTorque_ref = Ankle_Torque_from_cZMP(cZMP_TC_proj, ZMP_global, qRF_ref, qLF_ref, pRF_ref, pLF_ref, RS.F_RF, RS.F_LF, COM_m);
...
//    LF_landing_angle_gain_pitch = 12*50/(50 + RS.F_LF.z);
    ...
    if(Joint_deflection_compensation_flag == true){
        if(SDB[step_phase].swingFoot == RFoot){
            ...
            L_roll_compen_deg = alpha*L_roll_compen_deg + (1-alpha)*HB_sign(ZMP_local.y)*0.1*13;  //New change
        ...
        if(SDB[step_phase].swingFoot == LFoot){
            ...
            // hip roll
            double alpha = 1/(1 + 2*PI*dt*3);
            R_roll_compen_deg = alpha*R_roll_compen_deg + (1-alpha)*HB_sign(ZMP_local.y)*0.1*13;  //New change
...
void HB_PreviewWalk::MeasurementInput(RobotStates _RST){
    ...
    // ZMP calculation
    ZMP_global = ZMP_calc_global(pRF_ref, qRF_ref, RS.F_RF, RS.M_RF,
                                 pLF_ref, qLF_ref, RS.F_LF, RS.M_LF);
